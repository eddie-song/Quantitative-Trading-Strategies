...
# get data as csvs
tickers = ['COIN', 'GRAB', 'INTC', 'MSFT', 'NVDA', 'SEAT', 'VTLE']
df = yf.download(tickers, start='2015-01-01', end=datetime.today().strftime('%Y-%m-%d'))['Adj Close']

assets = tickers
weights = np.array([0.0477, 0.018, 0.239, 0.0986, 0.1017, 0.0071, 0.0274])
...
ef.add_constraint(lambda x: x >= 0.05)  # Minimum weight 5%
ef.add_constraint(lambda x: x <= 0.30)  # Maximum weight 30%
...
da = DiscreteAllocation(cleaned_weights, latest_prices, total_portfolio_value=1310.42)
